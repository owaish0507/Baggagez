...
export function PaymentOptions({ bookingData, onPaymentComplete, onPaymentError }: PaymentOptionsProps) {
  ...
  const handlePayment = async () => {
    ...
    setTimeout(() => {
      ...
      // Redirect to booking success page with all details
      const successUrl =
        `/booking-success/${bookingData.bookingId}?` +
        `paymentId=${paymentData.paymentId}&` +
        `amount=${bookingData.totalAmount}&` +
        `method=${paymentMethod}&` +
        `customer=${encodeURIComponent(bookingData.customerName)}&` +
        ...
        <CardContent className="space-y-6">
          ...
          >
            {isProcessing ? (
              ...
              `Confirm Pickup - ₹${bookingData.totalAmount}`
            )}
          </Button>
